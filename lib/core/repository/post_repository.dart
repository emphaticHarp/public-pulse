...
''')
      ...
      debugPrint('[DEBUG-REPO] getPosts: query returned ${response.length} raw posts');
      ...
      for (final json in response) {
        ...
        debugPrint('[DEBUG-REPO] getPosts: processing post id=${json["id"]}, owner=$postOwnerId');
        debugPrint('[DEBUG-REPO] getPosts: raw post JSON keys = ${json.keys.toList()}');
        ...
        final isPrivate = profile != null ? (profile['is_private'] as bool) : false;
        // Get this post's visibility setting (PUBLIC, FOLLOWERS, etc.).
        final visibility = json['visibility'] as String? ?? 'UNKNOWN';
        debugPrint('[DEBUG-REPO] getPosts: isPrivate=$isPrivate, visibility=$visibility, following=$following');
        ...
        if (!isPrivate) {
          ...
          if (visibility == 'PUBLIC') {
            debugPrint('[DEBUG-REPO] getPosts: adding PUBLIC post from public account');
            ...
            debugPrint('[DEBUG-REPO] getPosts: adding FOLLOWERS post (user is following)');
            posts.add(PostModel.fromJson(json));
          } else {
            debugPrint('[DEBUG-REPO] getPosts: SKIPPING post - visibility=$visibility, following=$following');
          }
        } else {
          // Private account Only followers can see any posts.
          if (following) {
            debugPrint('[DEBUG-REPO] getPosts: adding post from private account (user is following)');
            posts.add(PostModel.fromJson(json));
          } else {
            debugPrint('[DEBUG-REPO] getPosts: SKIPPING post from private account - not following');
          }
        }
      }

      debugPrint('[DEBUG-REPO] getPosts: returning ${posts.length} filtered posts');
      return posts;
    } catch (e, stackTrace) {
